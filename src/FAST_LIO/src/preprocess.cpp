  ...
  :feature_enabled(0), lidar_type(AVIA), blind(0.01), point_filter_num(1)
{
  ...
  N_SCANS   = 6;
  SCAN_RATE = 10;
  fov_azimuth = 180;   // 기본: 방위각 제한 없음(360°). yaml preprocess/fov_azimuth 로 변경
  group_size = 8;
  disA = 0.01;
  disA = 0.1; // B?
  p2l_ratio = 225;
    ...
    for(uint i=1; i<plsize; i++)
    {
      if((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
      {
        ...
        if (valid_num % point_filter_num == 0)
        {
          ...
          // 방위각(좌우) FOV 제한: 앞(+x) 기준 ±fov_azimuth° 밖이면 버림 (180이면 무제한)
          if (fabs(atan2(pl_full[i].y, pl_full[i].x) * 57.29578) > fov_azimuth) continue;
