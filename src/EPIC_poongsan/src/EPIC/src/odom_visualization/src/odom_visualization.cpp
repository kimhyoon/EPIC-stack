...
int main(int argc, char **argv) {
    ...
    if (!ros::param::get("/exploration_node/odometry_topic", odom_topic) ||
        odom_topic.empty()) {
        ROS_FATAL("[odom_visualization] /exploration_node/odometry_topic not set "
                  "(config yaml not loaded?). REFUSING TO START - no fallback.");
        return 1;
    }
    ROS_INFO("[odom_visualization] odom topic: %s", odom_topic.c_str());
    ros::Subscriber sub_odom = n.subscribe(odom_topic, 1000, odom_callback);
    ...
    //message filter
    message_filters::Subscriber<nav_msgs::Odometry> sub_quadstate_me(n, odom_topic, 1000); //本身
